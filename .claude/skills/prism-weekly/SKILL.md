...
- **비개발자 청중 기준으로 용어를 먼저 바꾼다** (사용자 지시 · 2026-09-09). 청중은 기획 · 운영 · 검수자다.
  코드 식별자(hold_fields · meta_n) · 개발 용어(폴백 · 게이트 · 홀드아웃 · 락 · 큐 · 슬롯 · 신뢰구간 · 직렬화 등)를
  `plain-terms.md` 표대로 회의용 표현으로 치환한다. 제품 용어(메타 · 리드문 · 정답셋 · 오토파일럿 등)와
  화면 문구 인용은 그대로. 새 용어는 표에 한 줄 추가한다.
- 그 뒤 slop-check 스킬로 전 슬라이드를 검수한다(도해 라벨 · 표 안 텍스트 포함). IM-KOREAN 에이전트에
  용어 표와 함께 맡기면 한 번에 끝난다(파일 직접 수정 · 태그 · 플레이스홀더 수 전후 검증).
