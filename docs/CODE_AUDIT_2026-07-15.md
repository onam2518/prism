...
| ✅ | P1-7·11 골든셋 오염 방지(관리자 골든 보존·빈 등급 승격 차단) | #181 |
| ⬜ | **P1-6** 프롬프트 인젝션 — 실모델 eval 병행 필요 | — |
| ⬜ | **P1-8·9** 학습코어(메타컴파일 클로버·팀 스코프 교차오염) — LEARNED 전역/팀 아키텍처 결정 필요 | — |
...
아래는 단독 수정이 부적절해 **의도적으로 남긴** 과제다(코드가 어려워서가 아니라 결정·검증·조율이 선행돼야 함).
상세 진단은 각 항목의 본문 섹션(아래) 참조.

**① 실모델 검증이 선행돼야 하는 것**
- **P1-6 프롬프트 인젝션**: `_content_block`·`meta_prompts.call_user` 가 본문을 구분자 없이 직결. 고치려면 모든 프롬프트의 콘텐츠 래핑(XML 태그/구분자)을 바꿔야 하는데, 테스트가 전부 mock 이라 실모델 추출 품질 회귀를 못 잡는다. → **실모델 eval 셋(골든셋 정합성 전후 비교)과 함께** 진행.

**② 아키텍처 결정이 선행돼야 하는 것**
- **P1-8 메타컴파일 클로버 · P1-9 팀 스코프 교차오염**: 근본 원인은 `PR.LEARNED`(전역 프롬프트)에 팀별 피드백을 동기화하는 구조. `sync_learned` 에 team 을 넘기는 국소 수정으로는 부족하고, **LEARNED 를 팀별로 분리할지**를 먼저 정해야 한다(프롬프트 파이프라인 전반 영향). learnops.py 를 다른 세션이 활발히 작업 중이라 조율 필요.

**③ 마이그레이션 계획이 선행돼야 하는 것**
- **P1-13 PostgREST 1000행 절단**: Range 헤더 페이지네이션 또는 서버측 집계(RPC)로 전환 — 조회 계층 광범위 수정.
- **P1-14 contents PK 전역 유일**: PK 를 (team_id, hash) 복합으로 — **DB 마이그레이션** 필요.
- **P1-15·16 비멱등 이중삽입·비원자 upsert**: idempotency 키/트랜잭션(RPC) 도입.

**④ 허브 파일(동시작업 충돌 주의)**
- **P1-12 데스크탑 판정 저장 실패 롤백**: `app.js`(허브·활발한 churn). 모바일(`mobile.js`)의 `_netFail` 패턴 이식.

**⑤ 정리·소규모(묶어서 처리 가능)**
- P2 견고성 20건(본문 P2 섹션)·P3 정리(메모리 누수·모바일 드리프트·인프라·문서·테스트 품질).

> 재개 시: 이 문서 상단 진행 현황표에서 ⬜ 항목을 고르고, 해당 본문 섹션의 `모듈:줄`·수정 방향을 따른다.
> P0·핵심 P1(라벨 정합성·LLM/임베딩·엔티티 보강·타임존·신뢰도 키·골든 무결성)은 이미 반영됨.
