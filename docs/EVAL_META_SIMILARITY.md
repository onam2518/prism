...
1. 표준 라이브러리만으로 되는 것부터: intent 샘플 F1 · 카테고리 계층 F1 · 엔티티 정규화 F1 을 `abtest.score` 에 추가하고 비교표에 열을 늘린다.
2. summary 임베딩 코사인(기존 `embed` 재사용).
3. 게이트 복수화 + 부트스트랩 신뢰구간(현재 이항 CI 대체).
4. 오토파일럿에 후보 모델 목록 인자.
