...
| 검수 인력 운영(HR) → **crewops.py** | `capacity` `profiles`/`set_profile` `crew_data` `plan_distribute` `rebalance` `escalate_split` `category_reliability` `auto_tick` `owner_rules`/`set_owner_rules`(서비스·주제별 전담) | /crew /crew-profile /crew-assign /crew-rebalance /crew-wave /crew-auto /crew-owner |
