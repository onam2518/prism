...
키를 넣으면 OCR · DocVision · 생성이 실모델로 동작한다(키 없으면 자동 mock). 키 입력 후 **모델 불러오기**를 누르면 키로 접근 가능한 생성 모델이 드롭다운으로 채워진다. `이 기기에 저장` 체크 시 `~/.prism_key`에 저장되어 재시작 후에도 유지된다. 키는 `config.json`에 저장되지 않는다.
