"""이미지 시각 슬롯 선택(콘텐츠 추가 · 미디어 탭) 회귀: per-call 오버라이드 · 순수 사진 라우터 폴백 ·
후보 목록 노출 · run_pipeline vision_used.

배경: 리포트(2026-07-22 · 샘플 20장)에서 기본 Upstage IE 는 순수 사진 장면 인식 0/20,
라우터 멀티모달(gemini-3.5-flash)은 20/20. 실험을 이미지 탭 기능으로 옮기며
시각 슬롯을 호출별로 고를 수 있게 하고, IE 가 순수 사진을 못 읽으면 라우터로 폴백한다.
(미디어 탭은 2026-08-13 실험실에서 콘텐츠 추가로 승격 · 백엔드 계약은 동일)
