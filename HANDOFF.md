...
다음 세션이 바로 이어갈 수 있도록 현재 상태를 정리한 문서. (갱신: 2026-07-09 · 세션 공통 규칙은 `CLAUDE.md`가 우선)
...
- **모바일 검수 /m**(7/9 · #74·#75): 위 '실행 방법' 참조. 시안(HTML 6프레임) → v1(카드
  ...
  dvh 폴백). ⚠️ 이원화 부채: FIX_ELEMENTS·INTENT_DEF 가 app.js 와 중복(검수 UI 정책 변경
  시 두 곳 반영).
...
2. **모바일 v1.1 운영 확인**: 아이폰에서 폰트·본문·한글 카테고리 재확인(본문이 비면
   "본문이 저장되지 않은 콘텐츠" 안내로 데이터/렌더 구분). v2 후보 = 골드 문항 카드 ·
   PWA manifest · 정의 시트 확장.
