...
다음 세션이 바로 이어갈 수 있도록 현재 상태를 정리한 문서. (갱신: 2026-07-15 · 세션 공통 규칙은 `CLAUDE.md`가 우선)
...
1. **운영 사이클 개시** ← 최우선(유지): 실배치 → 실행 → 퀘스트 → 검수(이제 모바일 /m 병행
   가능) → 학습 반영. 골드 문항 비율·미션 난이도·품질 배율·모델별 learned 실데이터 모니터링.
2. **모바일 v2 운영 확인**(2026-07-13 구현): 골드 문항 카드(/raw 필터 해제 · 응답 후 정오답
   공개·미션 보상 토스트) + PWA manifest(/vendor/m.webmanifest · 홈화면 설치) + 정의 시트
   확장(등급·품질 사유·카테고리 탭 정의 · 원천 /dict). 아이폰 실기기에서 홈화면 설치·시트
...
7. **엔티티 사전 후속**: 보류 296건 위키데이터 재시도(다음 일괄 보강) · topics_data 팀
