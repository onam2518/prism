...
다음 세션이 바로 이어갈 수 있도록 현재 상태를 정리한 문서. (갱신: 2026-07-28 · 세션 공통 규칙은 `CLAUDE.md`가 우선)

> **지금 막혀 있는 것**(2026-07-28): ① 라우터 잔액 소진으로 모델 실행이 전건 402
> ② #333·#334·#337 배포 대기(실행 중 배치가 끝나야 함). 상세는 아래 '다음 단계'.
...
1. **라우터 잔액 충전** ← 최우선. 충전 전까지 모델 실행은 전건 402 이고, 돌릴 때마다
   빈 값 행이 검수 목록에 쌓인다.
2. **배포 재실행**(#333·#334·#337): 진행 중 배치가 끝나면 Fly Deploy 재실행 → `/config`
   build·bootId 확인.
3. **빈 산출 복구 수단**: 실패 200건 중 건별 재실행 목록에 오른 건 100건뿐
   (`dashops._RECENT_FAIL_CAP`). 일괄 재실행 범위는 '미실행만'(해당 없음) / '전체'(과금 큼)
   둘뿐 → **'빈 산출만' 범위 신설** 검토. 미실행 200건도 별도로 남아 있다.
