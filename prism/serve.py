...
<script>
  document.addEventListener('alpine:init', () => {
    Alpine.data('prismApp', () => ({
      ...
      openDetail(c) { this.detail = Object.assign({ entities: [], intent: [], category: [], reasons: [], fb: {} }, c); if (!this.detail.fb) this.detail.fb = {}; this.editVerdict = false; this.pendingBad = false; this.detailOpen = true; this.drillOpen = false; },
      editVerdict: false, pendingBad: false,
    ...
    <div class="canvas">
      ...
      <div x-show="(mod === 'eval' && evalTop === 'queue') || mod === 'review'" x-cloak class="w-full space-y-4">
        <section class="panel" data-fn><div class="panel-hd"><b>검수 대기 목록</b>
          ...
          <div class="panel-bd">
            <ul class="ds-bullets" style="margin-bottom:11px"><li>자동(임베딩-LLM)이 확신 못 한 <b>YELLOW</b> 콘텐츠 대기열입니다.</li><li><b x-text="reviewer || '(이름 미설정)'"></b> 으로 검수하며, 여러 검수자의 의견은 모두 보존됩니다.</li><li><b>합의/불일치</b>로 실시간 집계됩니다.</li></ul>
    ...
    <div class="detailview" role="dialog" aria-modal="true" aria-label="콘텐츠 상세">
      ...
      <div class="detailview__body">
        ...
        <div class="detailview__eval">
          ...
          <div class="dve__sec"><div class="dve__lbl">카테고리</div><div class="flex flex-wrap gap-1"><template x-for="e in (detail?detail.category:[])" x-bind:key="e"><span class="ds-badge ds-badge--category" style="cursor:help" x-bind:data-tip="termDef('category', e)" data-tip-pos="right" x-text="e"></span></template><span x-show="detail && !detail.category.length" class="text-xs text-muted">·</span></div></div>
