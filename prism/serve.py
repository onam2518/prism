...
<style>
  ...
  *{-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility}
  ...
  .charcard__avatar{position:relative;width:140px;height:140px;margin:2px auto 8px;display:flex;align-items:flex-end;justify-content:center}
  .charcard__avatar::after{content:'';position:absolute;bottom:10px;left:50%;transform:translateX(-50%);width:78px;height:16px;border-radius:50%;
    background:radial-gradient(ellipse at center, color-mix(in srgb,var(--tier-c) 34%,transparent), transparent 72%);z-index:0;
    animation:charshadow 3.2s ease-in-out infinite}
  .charcard__avatar img{position:relative;z-index:1;width:92px;height:92px;transform-origin:bottom center;
  ...
  .charcard__glow{position:absolute;inset:6px;border-radius:50%;
    background:radial-gradient(circle,color-mix(in srgb,var(--tier-c) 40%,transparent),transparent 66%);
    box-shadow:0 0 0 3px color-mix(in srgb,var(--tier-c) 50%,transparent);opacity:calc(.35 + var(--tier,0)*0.15);
    animation:charpulse 2.8s ease-in-out infinite}
  @keyframes charfloat{0%,100%{transform:scale(calc(1 + var(--tier,0)*0.07)) translateY(0)}50%{transform:scale(calc(1 + var(--tier,0)*0.07)) translateY(-7px)}}
...
<div x-data="prismApp()" class="appshell" data-theme="light">
  ...
  <header class="topbar">
    ...
    <a href="/report" target="_blank" rel="noreferrer" class="ds-iconbtn ds-iconbtn--bordered topbar__pageicon" style="margin-left:14px" data-tip="전체 리포트 생성·보기" data-tip-pos="bottom" aria-label="전체 리포트"><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M6 3h8l4 4v14H6z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/><path d="M9 12h6M9 16h6M9 8h3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></a>
    ...
    <div class="canvas">
      ...
      <div x-show="mod === 'admin'" x-cloak class="w-full space-y-4">
        ...
        <section class="panel" x-show="adminData&&adminData.isAdmin"><div class="panel-hd"><b>골든셋</b><span class="meta" x-text="(adminData&&adminData.goldenCount?adminData.goldenCount+'건 등록됨':'미등록')"></span></div>
          <div class="panel-bd">
            <p class="text-xs text-muted" style="margin-bottom:10px">골든셋(정답셋). <b class="text-ink">{content, expected:{finalGrade, reasons}}</b> 형식의 .jsonl 을 올리면 교체 등록됩니다. (검증·평가 → 원천 평가에서 이 골든셋으로 정합성 측정)</p>
