...
# 인텐트 값의 정본은 사전(dictionaries) · 여기는 "사전 값 → 관점" 대응만 둔다(값 목록 재선언 금지).
# 아래 두 표는 tests/test_intent_labels.py 가 사전과 대조한다 —
# 현행 키는 사전에 있어야 하고, 레거시 키는 사전에 없어야 한다(이중관리·stale 동시 차단).
_ANGLE_CURRENT = {
    "속보·사건 추적": "속보", "속보·단신": "속보", "사건 경과 보도": "속보",
    "심층 분석": "분석", "해설·팩트체크": "분석", "비교·분석": "분석",
    "전술·데이터 분석": "분석", "트렌드·시장 분석": "분석",
    "의견·논쟁": "반응", "옹호·지지": "반응", "반박·비판": "반응",
    "의견·토론": "반응", "반응·리액션": "반응", "칼럼": "반응",
    "팬덤·화제성": "화제", "팬·응원 문화": "화제", "트렌드": "화제", "인물·사연": "화제",
}
# 레거시 별칭(읽기 전용): 2026-07-02 사전 개편으로 폐기된 옛 라벨.
# 삭제하지 않는 이유 — 토픽 집계는 저장된 과거 results 를 그대로 훑는데 옛 행의 item_meta 는
# 재추출되지 않아 폐기 라벨을 그대로 갖고 있다. 정규화에서 빠지면 같은 사건이 옛 라벨·현행 라벨로
# 쪼개져 앵글이 중복된다. 신규 부여 경로는 없다(프롬프트 주입·기계 검증 모두 현행 사전만 허용).
_ANGLE_LEGACY = {
    "속보": "속보", "단독": "속보",
    "기획·심층": "분석", "분석·해설": "분석",
    "의견·논평": "반응", "팬 반응": "반응",
...
ANGLE_MAP = dict(_ANGLE_LEGACY, **_ANGLE_CURRENT)
