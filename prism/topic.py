...
def _content_dims(rows, service_names, ent_index=None):
    """콘텐츠별 매칭 차원 사전계산: (Tier1 카테고리셋, 인텐트셋, 엔티티리스트, 개체속성리스트).
    ...
    return c_cat, c_int, c_ent, c_att
...
def build_topics(results_path: str, max_single: int = 200, max_composite: int = 120,
                 custom_defs=None, settings=None, ent_index=None) -> dict:
    rows = _read_jsonl(results_path)
