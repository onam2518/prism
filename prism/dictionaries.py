...
INTENT_VALUE_DEFS = {
    # 범용① 소비 방식 8종(2026-08-12 신규). 종전에는 이 8종에만 정의가 없어 프롬프트가 이름만
    # 주입했고, 모델이 자기 해석대로 붙였다 — 검수 지적의 60.4%가 인텐트였고 그중 오탐 1위가
    # '심층 분석'(27건)·4위가 '실용 정보'(16건)로 둘 다 여기 속했다.
    # 문구는 전부 검수자 메모 730건에서 뽑은 경계 규칙이다(지어낸 경계 없음).
    "속보·사건 추적": "막 발생한 사건을 처음 알리거나 그 후속 전개를 이어서 전하는 것 · 미부여: 정기 연재·분석 기사, 경기 결과 보도(스포츠 '경기 결과·리뷰'), 이미 끝난 사건의 정리·회고",
    "심층 분석": "사건·현상의 배경·원인·파급을 추가 취재나 자료 분석으로 파고든 것 · 그 취재·분석이 본문의 중심일 때만 부여 · 미부여: 발표·결과를 그대로 전하는 스트레이트 보도, 연구결과·보고서를 인용해 소개하는 기사(학술·전문 또는 해설·팩트체크), 시장·업계 흐름 분석(뉴스 '트렌드·시장 분석') · 글이 길거나 문단이 많다는 이유로 부여하지 않는다",
    "팬덤·화제성": "팬 반응·화제가 된 상황 자체가 글의 주제인 것 · 미부여: 이적·계약처럼 사실을 전하는 보도(스포츠 '이적·계약 보도'), 공식 발표 전달(연예 '캐스팅·공식소식' 등) · 출처가 UGC 라는 이유로, 또는 다루는 인물이 유명하다는 이유만으로 부여하지 않는다",
    "실용 정보": "독자가 바로 따라 할 수 있는 방법·조건·절차를 알려주는 것 · 미부여: 사실 전달이 목적인 소식(커뮤니티 '정보 공유'), 이적 소식·경기 일정 등 관심사 소비형 정보, 전문 지식·연구 현황 설명(학술·전문), 직접 체험에 기반한 평가(후기·리뷰·비평) · 콘텐츠뷰의 '생활·실용정보'와 같이 부여하지 않는다",
    "감성·공감": "읽는 사람의 감정을 움직이는 것이 목적인 것 · 개인의 경험·사연을 감정 중심으로 풀어낸 글에 부여 · 미부여: 인물의 뒷이야기를 사실로 전하는 것(TV '인터뷰·비하인드') · 검수 메모에 경계 사례가 5건뿐이라 이 값만 운영 합의 대기(2026-08-12)",
    "오락·유머": "정보 전달보다 재미가 목적인 것 · 소재가 정보성이어도 가볍게 즐기라고 쓴 글이면 부여 · 미부여: 사실·수치 전달이 목적인 글(커뮤니티 '정보 공유'·정형정보)",
    "의견·논쟁": "찬성과 반대가 병렬로 오가는 구조를 갖춘 것 · 사설·칼럼은 이 값을 기본으로 하되 한쪽 논조가 뚜렷하면 옹호·지지 또는 반박·비판을 우선한다(260715 회의 · 사설·칼럼 통합 운영) · 미부여: 가벼운 감상·소회 표현(티스토리 '에세이·산문')",
    "학술·전문": "글 전반이 전문 지식·연구 내용으로 이뤄진 것 · 미부여: 전공 지식이 일부만 인용되고 본문은 다른 목적인 글(비평·후기 등), 일반 독자를 향한 건강·생활 정보(실용 정보 또는 해설·팩트체크)",
    ...
    # 오탐 상위 값에는 '미부여' 경계를 명시한다(2026-08-12). 한 줄 정의만 있던 값들이
    # 검수 지적 258건(인텐트 지적의 58.5%)을 차지했다 — 정의가 없어서가 아니라
    # **언제 붙이지 말아야 하는지**가 없어서다. 문구는 검수자 메모에서 뽑았다.
    # '뉴스·소식' 으로 보내지 않는다 — 그 값은 콘텐츠뷰·커뮤니티 후보이고 뉴스에는 없다.
    # 없는 값을 가리키면 모델은 지시를 따를 수 없고 결국 원래 값을 그대로 붙인다(2026-08-12 발견).
    "정책·행정": "정책·사업 소개 + 법안 통과 + 행정 발표 통합 · 주최·발표 주체가 정부·지자체라는 이유로 부여하지 않는다 — 기사의 주제가 행정 자체일 때만 부여한다 · 미부여: 정부기관이 연 행사·전시 소식(행정 자체가 주제가 아니므로 서비스값을 비우고 범용값으로만 부여), 기업 홍보성 기사",
    "노동·사회 이슈": "노사·파업·고용 + 교통·산업·재난·생활 안전 통합",
    "인물·사연": "사람 자체(살아온 길·사연·됨됨이·별세)가 기사의 주제인 것 · 인물이 유명한지·어떤 신분인지로 가르지 않는다(M뉴스 사람·마음울림·오비추어리 유통풀 · 일반인이든 수상 작가든 삶·사연이 본문이면 부여) · 미부여: 인물의 직무 행위·발언·정책이 주제인 것(정책·행정 등 해당 주제 값), 수상·임명·선정 사실만 전하는 것(보도자료·공식발표), 문답만 있고 사연이 없는 것(범용② 인터뷰만 · 사연이 본문 축이면 병기)", "국제·외교 보도": "해외 사건·외교·국제 관계 전달",
    ...
    "뉴스·소식": "신제품·동향·공식 발표 등 소식 전달 · 정부·지자체가 주최한 행사·전시라도 기사의 주제가 행사 소식이면 이 값 · 미부여: 영상 클립·하이라이트가 전달의 주인 것(TV '클립·하이라이트')", "칼럼": "화자 관점이 강한 해설·주장(사설 포함 · 사설·칼럼 통합 운영)",
    ...
    "정보 공유": "알아두면 좋은 정보·팁·노하우를 나누는 것 · 경계: 단계별 절차 안내는 티스토리 '가이드·튜토리얼', 독자가 바로 따라 할 방법·조건은 범용 '실용 정보' · 같은 근거로 셋을 중복 부여하지 않는다", "의견·토론": "특정 이슈에 대한 의견 개진과 토론",
    "일상·잡담": "일상 공유, 가벼운 대화", "창작·작품 공유": "팬픽·그림·사진 등 창작물 공유",
    "에세이·산문": "개인 경험·감상 기반 글", "가이드·튜토리얼": "단계별 안내·설명 콘텐츠",
    "기술·개발": "프로그래밍·IT·기술 관련 글",
    "리뷰·분석": "직접 체험 없이 제품·서비스·콘텐츠를 정보·스펙·시장 관점에서 분석(보거나 써 본 뒤의 평가는 범용 '후기·리뷰·비평' · 관람·시청도 체험 · 같은 근거로 두 값을 중복 부여하지 않음)",
    ...
    "그래픽·인포그래픽": "도표·그래픽이 핵심 전달 수단",
    # 2026-08-03 운영파트 합의로 전면 교체(구: '이미지·영상 자체가 본문 · 캡션이 본문과 직접 연관될 때만').
    # 이 한 줄이 검수 호버·정책 도움말·모바일 정의 시트·프롬프트 사전 주입의 단일 원천이다.
    "포토·영상 중심": "직접 촬영한 것으로 읽히는 현장 사진이 콘텐츠의 핵심 전달 수단인 것 · 이미지 구좌(편성)에 넣을 콘텐츠 선별이 목적이라 사진 장수·본문 분량 같은 정량 기준으로 판정하지 않으며 사진이 한 장이어도 그 사진이 콘텐츠의 목적이면 부여(현장취재·화보·직접 촬영 인터뷰 포함 · 중간에 그래픽이 섞여도 현장·심층 취재면 부여) · 미부여: 글쓴이 프로필 사진이 대표 이미지인 칼럼·시론, 사진이 내용의 주가 아닌 것(해설·팩트체크), 직접 촬영이 아닌 사진만의 인터뷰, 생성형 AI 이미지만 있는 것, 사진 품질이 낮거나 중요하지 않은 것, 현장성 없는 단순 보도",
    "보도자료·공식발표": "공식 채널 발 발표·공고·캐스팅·스폰서·프로모션(광고성보다 우선 부여)",
    "후기·리뷰·비평": "글쓴이가 직접 사용·관람·체험한 것에 기반한 평가 · 영화·공연·방송을 보고 쓴 리뷰는 관람이 곧 체험이므로 어느 서비스든 부여(시승기·공연 리뷰·방송 리뷰·영화 리뷰·사용 후기) · 미부여: 보거나 써 보지 않고 정보·스펙·시장 관점으로만 쓴 분석(티스토리 '리뷰·분석' · 다른 서비스에서는 심층 분석 등 해당 범용값) · 같은 근거로 두 값을 중복 부여하지 않음",
    "해설·팩트체크": "쟁점 검증·배경 해설", "정형정보": "일정·부고·인사·시황·날씨·운세·기록 등 주기적·전달목적 정형 기사 · 같은 형식으로 되풀이 발행되는 것이 확인될 때만 부여한다 · 미부여: 보도자료를 받아 쓴 기사('보도자료·공식발표'), 재미 중심 유머('오락·유머')",
}

# 정의문 괄호 안의 다른 분류값을 읽는 법(게시판 #17 · 2026-08-25). 서비스 전용 값은 그 서비스 후보에만
# 실리므로(intent_categories_for · verify.py 화이트리스트) 범용 정의가 "커뮤니티 '정보 공유'" 를 가리켜도
# 뉴스에서는 고를 수 없다. 검수자가 "뉴스에도 정보 공유를 붙여야 하나" 를 물은 원인이 이 침묵이었다.
# 이 한 줄이 프롬프트(intent_dictionary_text)·검수 화면(/dict intentRefNote)의 공용 원천이다.
INTENT_REF_NOTE = ("정의문 괄호 안의 다른 분류값은 경계를 보여주는 이웃 개념 표시다 · "
                   "그 값이 이 서비스 후보에 없으면 붙이지 않고, 서비스값은 비운 채 근거가 있는 범용값만 부여한다")

# 범용① 소비 방식 8종의 정의는 2026-08-12부터 INTENT_VALUE_DEFS 가 단독으로 갖는다.
#
# 종전에는 여기(INTENT_UNIVERSAL_DEFS)에 UI 전용 정의가 따로 있었고 프롬프트에는 이름만
# 주입됐다 — 검수자와 모델이 서로 다른 것을 보는 구조였다(모델은 정의를 아예 못 봤다).
# 검수 지적의 60.4%가 인텐트였던 배경이다. v17 에서 INTENT_VALUE_DEFS 를 프롬프트에 주입하며
# 8종 정의를 그쪽에 채웠는데, /dict intentDefs 병합이 `{**UNIVERSAL, **VALUE}` 라
# 그 순간부터 UI 사전이 100% 덮여 죽은 값이 됐다. 사전을 둘로 두면 언젠가 반드시 갈라지므로
# 하나로 합쳤다. 옛 문구 중 살릴 것('실용 정보'의 관심사 소비형 정보 제외)은 옮겨 담았다.
#
# 검수 화면(/dict intentDefs)과 프롬프트가 이제 같은 문장을 쓴다 — 기준이 갈릴 수 없다.
...
_SERVICE_NAME_MAP = {
    ...
    # "포토"(imagext.build_content)·"영상"(mediaext.build_content·mediaops)은 의도적으로 없다.
    # 둘은 서비스명이 아니라 **인입 트랙 표식** — 호출자가 displayServiceName 을 안 줬을 때만
    # 붙는 기본값이다(UI 는 항상 /vocab 그룹값을 실어 보낸다). 서비스를 모르는 입력이므로
    # PGC 폴백(_service_key → "" · 범용①②만 부여)이 정확한 처리이고, 프롬프트도 이 경우를
    # 명시 문구로 다룬다("displayServiceName 미정의 → 범용 분류값만 부여").
    # 임의로 서비스 키를 붙이면 없는 서비스 분기 인텐트를 후보로 열어주는 오분류가 된다.
}
...
INTENT_EXAMPLES = {
    ...
    # O/X 경계가 드러나게(2026-08-03 합의) · 장수가 아니라 '직접 촬영한 현장 사진이 축인가'가 기준
    "포토·영상 중심": "O) [오늘의 1면 사진] 폭염 속 도심 분수대 — 사진 한 장이 콘텐츠의 목적 / X) 필자 프로필 사진이 썸네일인 시론, 자료 이미지가 여럿인 팩트체크",
}
