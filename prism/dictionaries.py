...
INTENT_VALUE_DEFS = {
    ...
    "속보·사건 추적": "막 발생한 사건을 처음 알리거나 그 후속 전개를 이어서 전하는 것 · 미부여: 정기 연재·분석 기사, 경기 결과 보도(경기 결과·리뷰), 이미 끝난 사건의 정리·회고",
    "심층 분석": "사건·현상의 배경·원인·파급을 추가 취재나 자료 분석으로 파고든 것 · 그 취재·분석이 본문의 중심일 때만 부여 · 미부여: 발표·결과를 그대로 전하는 스트레이트 보도, 연구결과·보고서를 인용해 소개하는 기사(학술·전문 또는 해설·팩트체크), 시장·업계 흐름 분석(트렌드·시장 분석) · 글이 길거나 문단이 많다는 이유로 부여하지 않는다",
    "팬덤·화제성": "팬 반응·화제가 된 상황 자체가 글의 주제인 것 · 미부여: 이적·계약처럼 사실을 전하는 보도(이적·계약 보도), 공식 발표 전달(캐스팅·공식소식 등) · 출처가 UGC 라는 이유로, 또는 다루는 인물이 유명하다는 이유만으로 부여하지 않는다",
    "실용 정보": "독자가 바로 따라 할 수 있는 방법·조건·절차를 알려주는 것 · 미부여: 사실 전달이 목적인 소식(정보 공유), 이적 소식·경기 일정 등 관심사 소비형 정보, 전문 지식·연구 현황 설명(학술·전문), 직접 체험에 기반한 평가(후기·리뷰·비평) · 콘텐츠뷰의 '생활·실용정보'와 같이 부여하지 않는다",
    "감성·공감": "읽는 사람의 감정을 움직이는 것이 목적인 것 · 개인의 경험·사연을 감정 중심으로 풀어낸 글에 부여 · 미부여: 인물의 뒷이야기를 사실로 전하는 것(인터뷰·비하인드) · 검수 메모에 경계 사례가 5건뿐이라 이 값만 운영 합의 대기(2026-08-12)",
    "오락·유머": "정보 전달보다 재미가 목적인 것 · 소재가 정보성이어도 가볍게 즐기라고 쓴 글이면 부여 · 미부여: 사실·수치 전달이 목적인 글(정보 공유·정형정보)",
    "의견·논쟁": "찬성과 반대가 병렬로 오가는 구조를 갖춘 것 · 사설·칼럼은 이 값을 기본으로 하되 한쪽 논조가 뚜렷하면 옹호·지지 또는 반박·비판을 우선한다(260715 회의 · 사설·칼럼 통합 운영) · 미부여: 가벼운 감상·소회 표현(에세이·산문)",
    ...
    "리뷰·분석": "직접 체험 없이 제품·서비스·콘텐츠를 정보·스펙·시장 관점에서 분석(직접 체험 기반 평가는 범용 '후기·리뷰·비평' · 같은 근거로 두 값을 중복 부여하지 않음)",
    ...
    "후기·리뷰·비평": "글쓴이가 직접 사용·관람·체험한 것에 기반한 평가(시승기·공연 리뷰·방송 리뷰·사용 후기 · 체험 없는 정보·스펙 분석은 티스토리 '리뷰·분석' · 같은 근거로 두 값을 중복 부여하지 않음)",
