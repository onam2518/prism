...
IMETA_VERSION = "imeta@v19"  # v19: 정의문이 다른 서비스 전용 값을 가리킬 때 서비스명 명시(정보 공유·경기 결과·리뷰·트렌드·시장 분석·이적·계약 보도·캐스팅·공식소식·인터뷰·비하인드·에세이·산문) + '읽는 법' 한 줄 주입(괄호 값은 경계 표시 · 이 서비스 후보에 없으면 서비스값 비움) + 인물·사연 경계(유명도가 아니라 사람 자체가 주제인가) + 후기·리뷰·비평은 관람·시청도 체험 + 실행 불가능하던 '경계 콘텐츠 양쪽 서비스값 동시 부여' 규칙 제거(게시판 #17) · v18: 오탐 상위 기존 값 4종(정책·행정·뉴스·소식·정보 공유·정형정보)에 미부여 경계 명시 · 정책·행정은 주최 주체가 아니라 기사 주제로 판정(검수 메모 근거) · v17: 범용① 소비 방식 10종 정의문 프롬프트 주입(종전 이름만 나열 · 8종은 정의 자체가 없었다 · 검수 지적의 60.4%가 인텐트였고 오탐 1·4위가 이 8종 · 문구는 검수자 메모 730건 근거) · v16: 포토·영상 중심 판정 기준 전면 개정(260803 운영파트 합의 · 260715 캡션-본문 연관 규칙 대체) + 인텐트 콜 입력 확장(title·본문 글자수·이미지 수·도입부) + 범용② 정의문 프롬프트 주입 · v15: 스타일&패션 Tier2 개명 · Personal Care→Beauty · Fashion Trends→Fashion(주간회의 후속) · v14: 후기·리뷰·비평/리뷰·분석 경계(주간회의 260722) · v13: 인텐트 출처 무관 원칙 + 실용 정보·팬덤 경계(게시판 #7) · v12: 인텐트 수량 상한 제거 · v11=게시판 피드백 · v10=260715 회의 개편
