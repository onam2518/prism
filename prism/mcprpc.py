...
프리즘에는 MCP 표면이 둘이다. 실험 프로덕트인 스펙트럼 관문(`/spectrum-gw`)과
...
4. **인증 실패는 기록하지 않는다.** `on_call` 은 인증을 통과한 요청에만 불린다(감사 O2 —
   스펙트럼 관문이 인증 실패마다 상태 파일을 재기록해, 틀린 키로 600번 두드리면 실사용
   기록 500건이 밀려났다).
...
`handle_body(key, data)` 는 본문을 이미 파싱한 호출자용이다(스펙트럼 관문처럼 MCP 와
