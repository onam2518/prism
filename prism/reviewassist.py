...
#    (여기서 닫히지 않는 부분은 `_ask_terms` 주석에 적었다.)
...
ASK_DAILY_MAX = 30                                # 사용자당 하루 질문 수(모델 호출 = 과금)
...
def ask_model() -> str:
    ...
    따로 둔 이유가 자기 판정을 자기가 변호하지 않게 하려는 것이라서다.
      ...
      ② `_SV.assist_model`: 설정 계층의 해석 함수(정상 경로)
      ③ `""`: ②가 아직 없는 동안의 임시 다리. `llm_for_model` 이 기존 설정 모델로 해석한다.
         ②가 들어오면 이 가지는 죽는다. **여기에 모델 이름을 적어 넣지 말 것**(설정과 갈라진다).

    ②를 하드 import 하지 않고 `_SV` 로 더듬는 이유: 설정 항목이 아직 머지 전이라 import 하면
    이 모듈이 통째로 못 뜬다. 이름만 맞으면 머지되는 순간 이 파일 수정 없이 붙는다."""
...
def _ask_terms(q: str) -> tuple:
    ...
    ⚠️ 여기서 닫히지 않는 부분: "이건 왜 R 이야" 처럼 **콘텐츠에만 답이 있는 질문**은 골드에서
    자료가 비고, 자료가 비면 빨리 끝난다. 이 층에서 더 할 수 있는 것이 없다. 없는 자료를
    지어내면 4번 규칙이 깨지고, 가짜 지연을 넣으면 그건 거짓말이지 방어가 아니다. 이건 한
    응답이 아니라 응답들의 분포에서 드러나는 성질이라(모듈 상단 '셋째' 경우와 같은 종류)
    이 계층의 테스트로도 잡히지 않는다. 남겨 두고 적어 둔다."""
    s = str(q or "")
