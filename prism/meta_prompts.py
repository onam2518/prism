...
- 서비스 분류값은 displayServiceName 으로 분기된 그 서비스의 값만 쓴다. 정의문 괄호 안의 다른 서비스 값
  (예: 커뮤니티 '정보 공유'·티스토리 '리뷰·분석')은 경계를 보여주는 표시일 뿐 여기서 고를 값이 아니다.
  그런 성격이면 서비스값을 비우고 근거가 있는 범용값만 부여한다(게시판 #17).
...
def intent_dictionary_text(display_service_name: str) -> str:
    ...
    lines = ["[읽는 법] " + D.INTENT_REF_NOTE, "", "[범용 ① 소비 방식 · 전 서비스 공통]"]
