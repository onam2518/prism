...
# 인텐트 → 사용자 말(검색 칩·카탈로그 표시). 값 목록의 정본은 사전(dictionaries)이고
# 여기는 표현만 담당한다 · tests/test_intent_labels.py 가 현행/레거시를 사전과 대조한다.
_INT_KO_CURRENT = {
    # 범용① 소비 방식 10종
    "속보·사건 추적": "지금 뜨는 속보만", "심층 분석": "깊이 있는 분석만",
    "팬덤·화제성": "가볍게 볼 화제만", "실용 정보": "일상 꿀팁 모음",
    "감성·공감": "마음이 따뜻해지는 이야기", "오락·유머": "웃음 충전 콘텐츠",
    "의견·논쟁": "여러 시각의 논평", "학술·전문": "전문 지식 깊게 보기",
    "옹호·지지": "지지 논조로 쓴 글", "반박·비판": "비판 논조로 쓴 글",
    # 범용② 형식·전달 8종
    "인터뷰": "인물 인터뷰만", "현장취재·르포": "현장에서 쓴 기사",
    "그래픽·인포그래픽": "그림으로 보는 정보", "포토·영상 중심": "사진·영상으로 보기",
    "보도자료·공식발표": "공식 발표 소식", "후기·리뷰·비평": "직접 써 본 후기",
    "해설·팩트체크": "차분한 해설 기사만", "정형정보": "일정·기록 정보",
    # 서비스 분기 중 데모 카탈로그에 자주 걸리는 값
    "사건 경과 보도": "사건 흐름 따라잡기", "인물·사연": "인물 소식만",
    "생활·실용정보": "일상 꿀팁 모음",
}
# 레거시 별칭(읽기 전용): 2026-07-02 사전 개편으로 폐기된 옛 라벨. 데모 카탈로그는 저장된
# 과거 추출 결과를 그대로 읽으므로 옛 행에는 폐기 라벨이 남아 있다 → 표기·검색 매칭 유지용.
# 신규 부여 경로 없음(추출은 현행 사전 값만 통과).
_INT_KO_LEGACY = {"기획·심층": "깊이 있는 분석만", "분석·해설": "차분한 해설 기사만",
