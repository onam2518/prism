...
window.PRISM_APP_PARTS.push(() => ({
      ...
      // 값 목록의 정본은 서버 사전(dictionaries.py INTENT_*) · 여기는 표시 문구만 담는다.
      // 사전에 없는 값은 caLabel 이 원문 그대로 노출한다(누락돼도 화면이 비지 않음).
      caIntentKo: {                        // 인텐트(현행 사전 라벨) → 사용자 말
        // 범용① 소비 방식 10종
        '속보·사건 추적': '새로 나온 소식', '심층 분석': '깊이 있는 분석', '팬덤·화제성': '가벼운 화제',
        '실용 정보': '생활 정보', '감성·공감': '공감 이야기', '오락·유머': '웃긴 이야기',
        '의견·논쟁': '의견·칼럼', '학술·전문': '전문 지식', '옹호·지지': '지지하는 글',
        '반박·비판': '비판하는 글',
        // 범용② 형식·전달 8종
        '인터뷰': '인터뷰', '현장취재·르포': '현장 취재', '그래픽·인포그래픽': '그림 정보',
        '포토·영상 중심': '사진·영상', '보도자료·공식발표': '공식 발표', '후기·리뷰·비평': '후기·리뷰',
        '해설·팩트체크': '해설', '정형정보': '일정·기록',
        // 서비스 분기 중 자주 노출되는 값
        '사건 경과 보도': '사건 진행', '인물·사연': '인물 이야기', '생활·실용정보': '생활',
      },
      // 레거시 별칭(읽기 전용): 2026-07-02 사전 개편으로 폐기된 옛 라벨 · /raw 에는 개편 이전에
      // 추출된 콘텐츠가 그대로 남아 있어 표시용으로만 유지한다(신규 부여 경로 없음).
      caIntentKoLegacy: {
        '기획·심층': '기획 기사', '분석·해설': '해설', '흥미·화제': '가벼운 화제',
        '의견·논평': '의견·칼럼', '인물 동정': '인물 이야기', '라이프스타일': '생활',
      },
      caLabel(kind, v) {                   // kind: topic|field|kind(내용)|ent
        ...
        if (kind === 'kind') return this.caIntentKo[v] || this.caIntentKoLegacy[v] || v;
        return v;
      },
