/* Prism 앱 스크립트(Alpine) · serve.PAGE 에서 분리된 단일 원천. */
  document.addEventListener('alpine:init', () => {
    Alpine.data('prismApp', () => ({
      ...
      studio: { name: '', prompt: '', cats: [], intents: [], keywords: [], eattrs: [], kwInput: '', eaKey: 'gender', eaVal: '', editId: null, auto: { cats: [], intents: [], keywords: [] }, req: { cats: [], intents: [], keywords: [] } },
      ...
      studioDef() { return { id: this.studio.editId, name: this.studio.name, prompt: this.studio.prompt, cats: this.studio.cats, intents: this.studio.intents, keywords: this.studio.keywords, eattrs: this.studio.eattrs, req: this.studio.req }; },
      ...
      // 조건 칩 3상태: off(후보) → sel(선택·관련 묶음) → req(필수·모든 묶음 공통) → off
      studioState(dim, val) { if (!this.studio[dim].includes(val)) return 'off'; return this.studio.req[dim].includes(val) ? 'req' : 'sel'; },
      studioCycle(dim, val) {
        const sel = this.studio[dim], req = this.studio.req[dim];
        const si = sel.indexOf(val), ri = req.indexOf(val);
        if (si < 0) sel.push(val);                       // off → 선택
        else if (ri < 0) req.push(val);                  // 선택 → 필수
        else { sel.splice(si, 1); req.splice(ri, 1); }   // 필수 → off
      ...
      chipCls(dim, val, base) { const s = this.studioState(dim, val); if (s === 'off') return 'ds-badge--neutral'; return base + (s === 'req' ? ' is-req' : ''); },
      ...
      bundleSummaryText() {
        ...
        return base + '핵심 <b>' + ((core && core.count) || 0) + '건</b>' + (rel ? (' + 관련 묶음 <b>' + rel + '개</b>로 펼쳐집니다') : ' (선택 조건을 더하면 관련 묶음이 생겨요)');
      },
      ...
      studioEdit(g) {
        ...
        this.studio = { name: def.name || g.name || '', prompt: def.prompt || g.prompt || '', cats: [...(def.cats || [])], intents: [...(def.intents || [])], keywords: [...(def.keywords || [])], eattrs: [...(def.eattrs || [])], kwInput: '', eaKey: 'gender', eaVal: '', editId: g.id, auto: { cats: [], intents: [], keywords: [] }, req: { cats: [...(rq.cats || [])], intents: [...(rq.intents || [])], keywords: [...(rq.keywords || [])] } };
        this.studioMsg = ''; this.studioPreviewNow();
        try { window.scrollTo({ top: 0, behavior: 'smooth' }); } catch (e) {}
      },
      studioReset() { this.studio = { name: '', prompt: '', cats: [], intents: [], keywords: [], eattrs: [], kwInput: '', eaKey: 'gender', eaVal: '', editId: null, auto: { cats: [], intents: [], keywords: [] }, req: { cats: [], intents: [], keywords: [] } }; this.studioPreview = { bundles: [], n_total: (this.topicData && this.topicData.n_contents) || 0, must_n: 0, opt_n: 0 }; },
