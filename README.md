...
[메타풀](https://onam2518.github.io/prism/demo-metapool.html) ·
...
리포트 HTML은 3개 탭으로 열립니다: **아이템 메타 · 메타풀 · 사용자 메타**.
...
**관계로 묶기 (메타풀)**: 흩어진 콘텐츠를 세 축으로 그룹핑.
- 단독형: 인물·기업 단위 ("손흥민 관련 전부")
- 복합형: 같은 사건을 자동으로 묶어 **대표 1건 + 관련 N건** (중복 제거·관점 분산)
- 필터형: 자연어 조건만 적으면 부합 콘텐츠가 자동 편입 ("경제 × 심층 분석"). **이 조건의 주체가 운영자면 큐레이션, 개인이면 개인 피드가 됩니다.**
...
| `report --batch/--results` | 추출→메타풀→대시보드 한 번에. `--profile`, `--logs`, `--demo` |
...
| 메타풀 · 단독형 | 단일 엔티티 단위 그룹("이 인물·기업 관련 콘텐츠"). 영속 |
| 메타풀 · 복합형 | 사건 단위 그룹(엔티티 공출현으로 자동 발견). 단기 |
| 메타풀 · 필터형 | 조건 단위 그룹(인텐트 카테고리 × 엔티티 카테고리). 중장기 |
