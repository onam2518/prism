...
| 정답셋 관리* | 정답셋 현황·학습 반영 · 정답셋 목록 · 학습 데이터(SFT/DPO/소요서) |
...
- 서버: 파이썬 표준 라이브러리 http.server + Alpine.js UI(마크업 `prism/page.py` · 스크립트/스타일 `prism/vendor/app.js`·`app.css` · 도메인 모듈 `learnops`/`adminops`). 외부 패키지 의존 없음.
