...
## 코딩 규칙(게으른 시니어 · prism-lazy)

모든 코드 작성·수정에 항상 적용한다. 원본은 [ponytail](https://github.com/DietrichGebert/ponytail) 이고,
프리즘 판 전체 규칙과 리뷰·감사 스킬은 저장소 플러그인 `plugins/prism-lazy` 에 있다
(`/prism-lazy` · `/prism-lazy-review` · `/prism-lazy-audit`).

코드를 쓰기 전 아래 사다리를 타고 **첫 번째로 성립하는 칸에서 멈춘다.**

1. 꼭 있어야 하나? 추측성 필요면 안 만든다. (YAGNI)
2. 저장소에 이미 있나? `ARCHITECTURE.md` 와 grep 으로 먼저 찾는다. 다시 짜지 않는다.
3. 파이썬 표준 라이브러리가 하나? 쓴다. (의존성 0 원칙)
4. 브라우저·SQLite 기본이 하나? `<input type>` · CSS · 제약 조건이 JS·앱 코드보다 먼저다.
5. 이미 들어와 있는 것(Alpine · 기존 모듈)이 하나? 쓴다.
6. 한 줄로 되나? 한 줄로 쓴다.
7. 그래도 안 되면 동작하는 최소 코드.

- 사다리는 문제를 다 읽고 흐름을 끝까지 추적한 뒤에 탄다. 읽기를 줄이는 게 아니라 해법을 줄인다.
- 버그는 증상이 아니라 원인. 호출자를 전부 grep 하고 공유 함수 한 곳을 고친다.
- 요청 없는 추상화·나중을 위한 뼈대·새 의존성 금지. 추가보다 삭제. 파일은 가장 적게.
- 일부러 자른 부분은 `# ponytail: <한계> · <올릴 때 방법>` 주석을 남긴다.
- 게으르면 안 되는 것: 신뢰 경계 입력 검증 · 데이터 손실 방지 · 보안 · 접근성 · 명시적으로 요청된 것.
- 분기·루프·파서가 들어간 논리는 깨지면 실패하는 테스트 한 건을 남긴다. 한 줄짜리엔 안 남긴다.
- 설명은 코드 뒤에 최대 세 줄(뺀 것 · 언제 넣을지). 사용자가 요구한 보고서는 예외.
...
- 새 도메인 기능은 serve.py 에 쌓지 말고 별도 모듈로 시작(`entdict.py`·`dictops.py` 패턴).
...
2026-09-02 세션 기록·PR 원장 측정 결과를 근거로 한 규칙. 낭비 상위는 400K 초과 컨텍스트 ·
한 턴 수십 건 도구 루프 · 캐시 만료 재작성이었고, 머지 PR 의 절반이 30일 안에 같은 파일을 다시 고쳤다.

1. **세션 하나에 PR 하나.** PR 을 만들었으면 후속 요청은 새 세션에서. (비용 귀속과 후속 수정 연쇄 방지)
2. **컨텍스트 300K 넘으면 멈춘다.** `/compact` 하거나 새 세션으로 넘긴다. 큰 조사는 서브에이전트에 맡겨 결과만 받는다.
3. **도구 결과를 통째로 싣지 않는다.** 20K자 넘는 출력 금지 · `head`·`grep`·`sed -n` 으로 자르거나 스크립트 안에서 요약해 출력.
   같은 파일을 세 번 이상 Read 하지 않는다.
4. **반복 호출은 스크립트 한 번으로.** 한 턴에 도구 호출 25건을 넘길 것 같으면 파이썬·셸 스크립트로 묶어 실행.
5. **한 시간 이상 자리를 비우기 전엔 세션을 마무리한다.** 돌아와서 이어 쓰면 컨텍스트 전체를 다시 캐시에 쓴다.
6. **측정**: 월 1회 `~/orca/claude-ledger` 의 `ledger.py audit --project orca/prism --since 30d` 와
   `prledger.py report` 로 기준선(`baselines/2026-09-02-prism.md`)과 비교한다.
